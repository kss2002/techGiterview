...
// 로컬스토리지 유틸리티 함수들
const storageUtils = {
  saveApiKeys: (githubToken: string, upstageApiKey: string, googleApiKey: string) => {
    try {
    ...
    try {
      if (useLocalStorageMode) {
        ...
        if (saveToLocalStorage) {
          storageUtils.saveApiKeys(githubToken, upstageApiKey, googleApiKey)
          console.log('API 키가 로컬스토리지에 저장되었습니다.')
        }
        ...
        <form onSubmit={handleSubmit} className="api-key-form">
          ...
          <div className="form-group">
            <label htmlFor="upstage-api-key" className="form-label">
              <Sparkles className="icon" /> Upstage API Key (Solar Pro3)
            </label>
            <input
              type="password"
              id="upstage-api-key"
            ...
            <div className="form-help">
              ...
              <br />
              <small>Solar Pro3 사용을 위한 키가 필요합니다</small>
            </div>
          </div>

          <div className="form-group">
            <label htmlFor="google-api-key" className="form-label">
              <Globe className="icon" /> Google API Key (Gemini, 선택)
            </label>
            <input
              type="password"
              id="google-api-key"
